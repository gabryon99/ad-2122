...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Can we bound the error with a certain probability? \textit{Suggestion}: it does not suffices to say that it is at most $\delta$ 
...
\noindent Therefore, given an universe $U$ we can build a collection of \textit{dyadic ranges}, and we will
need at most $2n$ of them. Having these ranges, we can bound the error of the range query up to $\log n$, thus a
logarithmic error.
...
\noindent This can be achieved using $\log n$ Count-Min Sketches. We build a ``logic'' binary tree where each range is splitted in two parts.
...
\noindent When we habe to update a counter, we traverse the tree updating the counters for each traversed range.
...
The \verb+update+ operation takes $O(\log n)$ time, the pseudo-Python code is shown in Listing ?.
...
\noindent The random variable $X_{(a, b)}$ is a value representing the ``garbage''. We would like to know the expected value of this random variable.
...
\noindent Knowing that $E\big[X_{i}^{(j)}\big] \le 2n \frac{\epsilon}{e} ||F||$, we can bound the error probability using the Markov's inequality:
...
A precise analys should involve the fact that the ``queryable'' ranges are not only the dyadic ones. If we use more than one Count-Min Sketch as explained 
